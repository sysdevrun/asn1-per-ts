...
2. **Low-level**: Individual codec classes (`src/codecs/`) + `BitBuffer` (`src/BitBuffer.ts`) - decode directly from a bit-level buffer.
...
| `src/codecs/Codec.ts` | `Codec<T>` interface with `decode(buffer)` method |
